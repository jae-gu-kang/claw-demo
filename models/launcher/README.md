...
## 개정 2026-09-02 — 2.0배 확대 · 단일 개구

기체(전장 3.5 m·스팬 2.5 m)가 캐니스터 박스에 들어가는 비례로 다시 구웠다(사용자 지정
2~3배의 하한 쪽 — 균일 확대는 캐니스터 바닥도 올려 시뮬 발사 원점(1.2 m)과의 높이
어긋남이 배율에 비례해 커지므로 필요한 만큼만). 방법은 `LAUNCHER_Root` 노드 스케일
2.0이고 **메시 로컬 좌표는 그대로**다 — 런타임 관절 값·노드 이름 불변. 포구의 2×2 관
4개는 단일 개구(로컬 ±0.66×±0.44, 확대 후 2.6×1.8 m)로 바꿨다 — 스팬 2.5 m가 지나갈
수 없는 관은 발사 순간 앞면을 뚫는 그림이 됐다. 시뮬 `origin_height`를 캐니스터 축
(≈3.6 m)으로 올리면 기체가 관 속에서 나온다.

## 개정 2026-09-02 (2차) — 확대 상쇄 버그 수정 · 수평 2배 / 높이 인하 · 트러니언 후방 이동

1차 개정의 루트 스케일 2.0은 **화면에 살지 못했다.** `root.scale`을 부모화 앞에 걸어
`parent_to`의 `matrix_parent_inverse`가 0.5를 캡처했고, glTF에 `Trailer` scale 0.5로
실려 ×2가 통째로 상쇄됐다(발사관이 계속 1배로 보인 원인). 스케일을 부모화 **뒤로**
옮겨 고쳤다 — 지금은 역행렬이 단위행렬이라 ×2가 전 계층에 산다.

기하 자체도 다시 팠다(사용자 지정: 높이는 낮추고 나머지는 2배).

- **수평 2배** — 캐니스터 외피 5.8×7.6 m(개구 5.3×1.7 m·내부 길이 7.2 m), 트레일러
  상판 4.1×17 m, 아웃리거·드로바 동배율. 기체(스팬 2.5·전장 3.65 m)가 관 속에 통째로
  들어간다. 바퀴는 수평만 2배면 타원이 되므로 반지름은 원형 그대로 두고 **2축 4륜**으로
  벌렸다.
- **높이 인하** — 상판 0.92→0.80, 트러니언 1.78→1.45 (지면 기준 캐니스터 축
  3.56→**2.90 m**). 시뮬 발사 원점(1.2 m)과의 어긋남도 그만큼 준다.
- **트러니언 후방 이동** — 피벗을 관 뒤끝 0.3 뒤(로컬 y=−1.80)로. 7.6 m 관이 중앙
  피벗으로 들리면 고각 15°에서 이미 뒤끝이 상판을 뚫는다(선회 반경 4 m). 턴테이블
  중심도 y=−0.90으로 물려 기둥이 디스크 위에 선다.

지면 기준 실측(글TF bbox × 스케일): 캐니스터 관 7.22 m · 상부 레일까지 9.66 m ·
고각 15° 포구 높이 4.70 m. 시뮬 `origin_height`를 캐니스터 축(≈2.9 m)으로 올리면
기체가 관 속에서 나오는 그림이 된다. 노드 이름·관절 규약은 불변.
