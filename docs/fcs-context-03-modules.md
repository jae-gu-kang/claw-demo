...
- 비행모드: **선언적 모드 테이블 + 실행기(sequencer)** (01§3.3.1) — 각 모드 {진입조건, 활성 명령, 이탈조건}. 발사→상승→순항→접근→플레어→미끄럼→정지
- 종방향 지령은 **고도·피치·강하율 중 하나**(배타, `validate_longitudinal`) — 셋 다 θ_cmd로 가므로 우선순위 대신 구성 시점 거부다. 이탈 조건에 `hdot_ge`/`hdot_le`·`on_ground`/`airborne`·`off_rail`이 있고, 뒤 셋은 항법에 없는 정보라 sim이 ctx로 주입한다(없으면 **판정 불가**로 멈춘다 — False로 눙치면 모드가 조용히 선다)
- 고도 축이 경로를 **잡는 순간** `Guidance`가 상승엣지를 잡아 `LosPath.begin_alt_leg(nav)`를 부른다 — 구간 램프를 그 자리에서 다시 긋는 것이라, 축이 늦게 켜져도 명령이 기체 고도에서 연속으로 이어진다(01 §3.3). 헤딩에는 대응물이 없다: LOS 방위는 매 스텝 현재 위치에서 다시 재므로 시작점을 기억하지 않는다
...
- 구현됨(`pipeline/criteria.py`·`pipeline/evaluate.py`·`analysis/schedule.py`·`plant DispersionSet`, v0.55~v0.56): **게인 평가 A/B/C 체계** — A급 카드 7(ζ·ωn/GM/PM/BW·ω_gc/Ts·Mp/RMS/제어권한)·B급 체크 9(요약 판정)·C급 검증(`verify()` — 강건성 코너 재트림·격자 중간점). 기준은 GainEvalCriteria(schema v2, MarginCriteria+TuneTargets 합성, 지문 발급 — 02 §5.4 "합격기준을 파라미터로"의 결선), 하드 게이트는 코드 상수(문턱만 데이터). METRICS 12→27(tier/group 메타 — 키 불변). 절대 판정 조성은 M17 closure 재사용(§M17 의도적 이탈 기록 참조), 평가 프리미티브가 복수 게인 최적화기의 적합도 함수 이음새다. 상세·결정 근거는 02 §4 확정 블록과 v0.55~v0.56 이력
...
### M14 `web` — 프론트엔드
- **[확정]** 스택: 바닐라 ES 모듈 + no-build. 의존은 3D 월드 렌더링용 vendored three.js **1건**을 빼고 0이며, 그 예외의 대가로 렌더러 경계를 갈라 둔다 (02 §4 — CSP·폐쇄망 현지 수정·반입 심사 근거). 서버 StaticFiles 정적 서빙 = 배포
