...
### M5 `plant` — 비행체 모델
- 서브패키지: `eom`(6DOF 운동방정식+적분기), `aero`(공력, M3 소비), `prop`(스로틀-추력 맵, 쌍발·차동추력), `mass`(연료 의존 질량·CG·관성), `ground`(스키드 접촉·발사 레일 — 01 §3.3.1), `actuator`(2차계 가정값 시작, rate/position limit·backlash), `sensor`(초기 이상센서 → 노이즈·지연)
