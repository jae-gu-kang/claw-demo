...
- **엔벨로프 수치**(`analysis/envelope.py`): V-n 경계(`vn_envelope`·`vn_stall_boundary`) + 설계 엔벨로프 M-h 합성(`design_envelope` — 01 §2.6, 행별 경계 승자 귀속)·공력 선도(`aero_envelope`)·mach 경계 단일 정본(`stall_mach_lo`·`row_machs` — M17 `design/grid.py`가 호출, 종전 grid 사유 `_mach_lo`의 승격)
