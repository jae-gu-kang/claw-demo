                     ...
                     double att_err, double rate)
{
    ...
    const double pid_hi_x = (pid_raw > pid_axis) ? pid_raw : pid_axis;
    const double pid_lo_x = (pid_raw < pid_axis) ? pid_raw : pid_axis;
    if ((pid_hi_x > prm->pid_out_hi && pid_inc > 0.0) || (pid_lo_x < prm->pid_out_lo && pid_inc < 0.0)) {
        pid_inc = 0.0;
    }
