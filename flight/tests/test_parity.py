...
def test_분할해도_지문은_그대로다():
    ...
    # 변경이라 지문을 안 움직였다.)
    # 앞선 갱신은 **엘레본 제어권한 배분** — 선회 하중만큼 피치 몫을 먼저 떼고
    # 남은 것을 롤이 수요만큼 가져간다(graphs.py _roll_budget_nodes). 축 순서가
    # 롤→피치로 바뀌고 배분 노드 12개가 늘어 그래프 위상이 달라졌다.
    # 이번 갱신은 **ki = 0 적분 경로 폴딩**이다(emit_c.py _pid_has_integrator) —
    # 헤딩·요축은 ki가 상수 0이라 적분기·증분·안티와인드업 가드가 영구 도달 불가
    # 코드였고, DAL A 구조적 커버리지(MC/DC 100%)가 원리적으로 닫히지 않는 자리라
    # 생성에서 뺐다. 출력식은 `+ 0.0` 소거뿐이라 제어법칙은 안 바뀌지만(±0.0 합
    # 무영향 — kd 폴딩과 같은 논거), 파라미터 목록(*_pid_ki)이 줄어 지문은 움직인다.
    # 앞선 갱신은 **축별 동압 상한**이다(fcl/demo.py _cap_for) — 롤 3자리만 상한
    # 4.0을 써서 M0.42 아래 룩업 값이 커졌다. 구조 변경이 앞선 것들과 다르다:
    # 그래프 위상도 파라미터 목록도 그대로이고 **룩업 표의 수치만** 바뀌었다
    # (fcl_data.c의 sched_roll_* 3개, 피치는 무변경). 데이터도 탑재 C에 박히므로
    # 지문은 정직하게 움직인다 — 형상의 신원에는 값이 포함된다.
    # 이번 갱신은 **v1.10 200 kg급 예제**다 — generate.py가 쓰는 제품 예제가 바뀌어 자동조종 경로 게인·스케줄 마하
