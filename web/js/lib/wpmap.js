...
// (사용자 제기). 로그 줌 기준으로 둔하게 잡되, 그 값은 사용자 요청으로 두 번
// 조정됐다: 100(너무 둔함) → 20(5배 올림) → **2**(다시 10배 올림, 사용자 제기).
// 지수만 고치면 되도록 남긴다 — 나눗수를 키우면 둔해지고 줄이면 예민해진다.
//
// 2는 **휠 한 눈금당 1.2^0.5 = 9.5%**다. 트랙패드 한 제스처(수십 이벤트)에서는
// 여전히 여러 번 곱해지므로, 그쪽이 너무 빠르면 다시 올릴 자리는 여기 하나다.
export const WHEEL_ZOOM_DIVISOR = 2;
