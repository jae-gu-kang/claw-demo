...
export function defaultWpRows() {
  return [axisWp(2600), axisWp(3300)];
}
...
export const DEFAULT_FORM = Object.freeze({
  ...
  // 기본 미션은 100 s 안팎에 선다. 200은 그 두 배 여유다 — 짧으면 서기 전에 끊긴다.
  // 정확한 시각은 실행 후 착륙 요약이 말한다(여기 적어 두면 조용히 낡는다).
  tEnd: "200",
  // 도달 반경 [기본값] — 100 m. **너무 작으면 경로가 끝나지 않는다**: 순항 88 m/s·
  // 뱅크 한계 0.7 rad에서 선회 반경이 940 m라 LOS 추종이 임의로 작은 원을 못 잡고
  // 목표를 지나쳤다 되돌기를 반복한다. 그 하한은 웨이포인트 기하 × 선회 성능의
  // 함수라 상수가 아니다 — 화면이 미리 판정하지 않고 사유를 적어 둔다.
  accept: "100",
  navOn: true,
  seed: "11",
  actOn: true,
