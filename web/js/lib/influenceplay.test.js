...
test("cycleAt: 되감기 직전에만 페이드 — 재생 구간을 먹지 않는다", () => {
  ...
  // 범위가 아니라 **정확값**이다. 0.4~0.6으로 두면 램프가 어긋나는 변이가 그대로
  // 통과한다. 아래 두 줄이 나눠 맡는다 — 어느 줄이 무엇을 잡는지 헷갈리면 한쪽을
  // 중복으로 보고 지우게 되므로 적어 둔다:
  //   · **분모만** 늘리는 변이((total-u)/(fw*1.2))는 창 시작이 2700에 그대로 있어
  //     위 2700 줄을 통과하지만, 진입 직후 fade가 1에서 0.833으로 **툭 떨어진다** —
  //     이 모듈이 없애려고 만든 하드컷이 페이드 안으로 옮겨 오는 셈이고, 이 줄이 잡는다
  //   · **fw 자체**를 늘리는 변이는 창이 2640으로 앞당겨져 연속이지만 2700에서
  //     fade가 0.833이 되므로 **위 2700 줄**이 잡는다
  // 150/300은 부동소수점에서도 정확히 0.5다(2850·5850 양쪽 다 === 0.5).
  assert.equal(cycleAt(2850, cfg).fade, 0.5);
  // fade는 알파에 곱해지는 값이라 1을 넘으면 안 된다. 지금은 withAlpha가 rgba()에
  // 그대로 넣어 CSS가 클램프하지만, 창 시작만 넓히는 변이(u > total - fw*2)는
  // 2500에서 fade를 1.667로 만들면서 262건을 전부 통과한다 — 나중에 fade가 알파가
  // 아닌 곳(반지름·lerp 가중치)에 쓰이는 순간 실제 버그가 된다
  assert.equal(cycleAt(2500, cfg).fade, 1, "페이드 창 밖은 정확히 1 — 1을 넘지 않는다");
  assert.ok(cycleAt(2999, cfg).fade < 0.01);
  // **되감김 너머**도 밟는다. 주기 안에서만 재면 fade가 u 대신 누적 경과를 읽는
  // 변이가 그대로 통과하는데, 그 변이는 첫 되감김 뒤 fade를 0에 고정한다.
  // fade는 간선·노드·밑칠·입자·도착 링 알파에 전부 곱해지므로 결과는
  // **두 번째 주기부터 원뿔이 영영 안 뜨는 것** — 기본 동작이 무한 반복이다
  assert.equal(cycleAt(3100, cfg).fade, 1, "되감김 뒤 페이드는 주기마다 1로 돌아온다");
  assert.equal(cycleAt(5850, cfg).fade, 0.5, "페이드 램프도 주기마다 되풀이된다");
