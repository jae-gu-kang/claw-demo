...
지켜지나". 그래서 이 탭의 패널은 기능 묶음이 아니라 **계층 파이프라인**이다:
...
다섯 층"이라고 말하게 된다 — 대신 그 패널은 **그 층이 무엇이고 어디서 나오는지**와
...
/** 6계층 — 패널의 순서가 곧 설계 순서다 (파이프라인). 각 층의 정의는 여기 한 곳.
...
/** 층 머리 — 정의 한 문단 + "이 도구에서는" 한 줄. 여섯 패널이 같은 얼굴이어야
...
export function render() {
  ...
  // ── 필요값 입력 — 패널 안. 한 번 정하면 잘 안 바뀌는 값들이라 늘 펴 둘 자리가
