...
1·2·3단계를 갖게 되면서(당시 이름은 「선형 훑기 / 평가 실행 / C급 검증」, 지금은
v0.71의 「선별 / 평가 / 검증」이다) 같은 탭에서 **3이 두 뜻**이 됐다 — 판독대의 3단은
폐루프 스윕이고 평가의 3단계는 섭동 검증인데, 감도를 채우는 것은 앞의 것뿐이다.
번호는 순서만 말하고 무엇인지는 말하지 않으니, 화면 문자열에서는 **이름**을 쓴다:
구조 · 개루프 마진 · 폐루프 실측, 그리고 스캔 · 스윕 · 구간 경향. 아래 주석의 단
번호는 코드 안 호칭으로 남겨 둔다(파이프라인 순서를 말할 때 여전히 짧다) — 화면에
나가는 문자열에만 이름을 쓴다.

## 구간 경향 (3단 C) — "전 구간에서 어느 쪽으로" (v0.52)

3단 B가 답하지 못하던 자리다. B의 요약은 런별 **최악 한 칸**만 내고(엔벨로프를
접는다), 케이스×런 전체 표는 18케이스 × 9런 = 162행이라 경향이 행 사이에 흩어진다.
...
export function render() {
  ...
  function renderOpenloop() {
    ...
    // 케이스별 전체 표는 접힌 근거로 남는다 (전 행을 펼치면 18케이스 × 루프 수가 된다)
  ...
  // 전체 표는 사실을 다 갖고도 18케이스 × 9런 = 162행이라 경향이 행 사이에 흩어진다
  ...
  /** 세 단계 한 장 (v0.71) — 등급(A·B·C)을 걷어내고 **단계**만 남긴다.
   *
   *  종전 표는 A·B·C를 "비용 구조"라고 설명했는데 정작 A와 B는 **같은 실행**에서
   *  같이 나온다(비용이 같다). 그래서 등급과 단계가 축이 둘인데 C만 양쪽에 걸쳐
   *  있어, A·B·C가 마치 비용 순서인 것처럼 읽혔다. 축을 하나로 줄인다: 번호는
   *  **무엇을 더 계산하느냐**만 가리키고, 대표/보조(카드냐 요약 한 줄이냐)는
   *  번호가 아니라 화면 표시로만 남는다.
   *
   *  이름이 **각 단계가 답하는 질문**을 말하는 것이 요지다. 종전 「1단계 · 선형
   *  훑기」는 평가의 첫 단계처럼 읽혔는데, 그 넷(ζ·GM·PM·λ_roll)은 **자동설계가
   *  이미 판정하는 바로 그것**이다(design/criteria.py, design/tune.py — 자동설계는
   *  6DOF를 한 번도 안 돈다). 이 탭이 있는 이유인 시간축은 전부 2단계에 있다.
   *  1단계만 돌고 "평가했다"가 되면, 자동설계를 통과하고도 추종 RMS가 한계의
   *  3~4배인 형상이 「이상 없음」으로 읽힌다 — 실제로 계측된 자리다.
   *
  ...
  function stageTable() {
    ...
    const rows = [
      ["1단계 · 선별", "선형 기준은 통과하나?",
        "트림 + 선형화 (6DOF 런 0)",
        "모드 안정성·GM·PM·응답속도 · 전체 극점·시간지연 여유·스케줄 전이 · " +
        "제어권한의 트림 소모분"],
      ["2단계 · 평가", "시간축에서 실제로 어떤가?",
        "케이스마다 표준 기동 런 + 동시명령 런",
        "과도응답·추종 오차 RMS·잔여 권한 · 상승시간·정상상태 오차·교차축·" +
        "실속 마진·포화 지속·포화 회복"],
      ["3단계 · 검증", "그 판정이 섭동에도 버티나?",
        "코너마다 기체를 다시 만들어 재트림",
        "새 항목 없음 — 위 전부를 강건성 코너(질량·Cmα·Cmq ±)·격자 중간점에서 다시"],
    ];
    return el("div", {},
      el("div", { class: "scroll-x" }, el("table", {},
        el("thead", {}, el("tr", {},
          ["단계", "답하는 질문", "새로 얹는 계산", "그래서 값이 생기는 것"]
            .map((h) => el("th", {}, h)))),
        el("tbody", {}, rows.map(([g, q, cost, gets]) => el("tr", {},
          el("td", {}, el("strong", {}, g)),
          el("td", {}, q),
          el("td", { class: "hint" }, cost),
          el("td", { class: "hint" }, gets)))))),
      el("p", { class: "hint", style: "margin:6px 0 0" },
        "1단계가 보는 넷(ζ·GM·PM·응답속도)은 ", el("b", {}, "자동 설계가 이미 판정하는 것"),
        "이다 — 자동 설계는 6DOF를 한 번도 안 돈다. 그래서 1단계는 평가의 첫 단계가 " +
        "아니라 평가 앞의 문지기다: 게인 탭에서 손으로 고쳐 자동 설계를 안 거친 " +
        "형상을 6DOF 런 전에 수십 ms로 거른다. ",
        el("b", {}, "이 탭이 있는 이유(시간축)는 전부 2단계에 있다"),
        " — 자동 설계를 통과하고도 추종 RMS가 한계의 3~4배인 설계점이 실제로 있었다."),
      el("p", { class: "hint", style: "margin:6px 0 0" },
