...
function buildTopSvg() {
  ...
  // 기체 → 항법. 블록 **안쪽**(u=1096)으로 내린다 — 바깥(1200)으로 내리면 판 ①의
  // 책등 라벨(배지 ①·"트림 · 선형해석", P(1206,344))을 정통으로 가로지른다.
  // 배선은 아래로 갈수록 화면에서 왼쪽으로 흐르므로(투영), u를 줄여 라벨 오른쪽으로
  // 충분히 물린다 — 1112는 라벨 오른쪽 끝을 2px 스쳤다(브라우저 실측)
  s += wire([[1096, 317, 60], [1096, 344, 60], [1096, 344, 48], [1096, 383, 48]]);
  ...
  s += label(1088, 369, 48, "x", "siglabel", "end");
