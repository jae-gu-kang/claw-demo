/** 고정 구조 신호흐름 다이어그램 (06 §1 (a)) — 층 보드 스타일 SVG 최상위.
...
"이 블록"을 강조할 설계변수도 없다. 껍질 하나를 .body로 두면 둘 다 해결된다. */
function hull(ps) {
...
function buildTopSvg() {
  ...
  // 기체 → 항법. u는 **두 조건 사이**에서만 성립한다 (브라우저 실측으로 좁힌 값):
  //   u ≥ 1106  끝점이 항법 블록 상단 모서리 안에 들어온다. 배선은 판 높이(z=48)에
  //             있고 블록은 그 위로 떠 있어(BT), 끝점 v(383)와 블록 모서리 v(413.5)의
  //             차이만큼 x가 어긋난다 — 1096에서는 9.6단위 바깥이라 안 닿았다
  //   u < 1128  판 ①의 책등 라벨(배지 ①·"트림 · 선형해석", P(1206,344)) 오른쪽으로 빠진다
  // 바깥(원래 1200)은 라벨을 정통으로 가로질렀고, 안쪽으로 너무 밀면 블록을 놓친다
  s += wire([[1120, 317, 60], [1120, 344, 60], [1120, 344, 48], [1120, 383, 48]]);
  ...
  s += '<g class="labels">';
  // 임무프로파일 — 화살표(v 156~213) 중간쯤. v를 그대로 반으로 나눌 수 없다:
  // 이 배선은 뒷줄(v<172)에서 앞줄로 넘어오며 DROP 절벽(sv가 +70 점프)을 가로지르므로
  // 화면 중앙에 대응하는 v가 아예 없다. 절벽 바로 아래(173)가 가장 가까우면서
  // 계획 값 슬롯(v 170.5)과도 떨어진 자리다 — 절벽 위(172)는 값과 겹친다
  s += label(126, 173, 24, "임무프로파일", "siglabel", "start");
  ...
  s += label(1112, 358, 48, "x", "siglabel", "end");  // 화살촉(383)에서 떨어뜨린다
