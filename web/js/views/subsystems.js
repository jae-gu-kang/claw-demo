  ...
  <text class="canvas-note" x="24" y="284">※ 입력은 SurfaceCommand 규약 (2,)를 그대로 받되 <tspan style="font-weight:700">평균만</tspan> 쓴다 · 교차속도 V_c = ηP/T_static = 66.7 m/s — 그 아래는 정지추력 상한, 위는 1/V</text>
        ...
        flow: {
          ...
          why: [
            ...
            "<b>1/V가 이 그림의 요점이다.</b> 상수 추력 모델은 고속에서 추력이 남아돈다고 말하지만 프로펠러는 정확히 거기서 힘이 빠진다. 그래서 엔벨로프 상단을 정하는 것이 항력이 아니라 <b>추력</b>이 되고, 이 기체의 수평비행 범위가 해면 M0.20~0.60(연료 200 kg — 만재 400 kg면 M0.22~0.58)로 닫힌다(설계 천장 연료 200 kg ~5.5 km · 만재 ~3.8 km · 공허 ~7.5 km). 엔벨로프 탭의 스로틀 포화 전선이 대리 지표가 아니라 추력 한계인 이유가 이것이다 — 다만 그 선은 스로틀 <b>95%</b> 등고선이라(trim.py SAT_FRAC) 진짜 한계보다 설계 여유만큼 안쪽이다.",
...
<ul>
  ...
  <li>스로틀-추력: <span class="mono">T = δ·σ·min(T_static, ηP/V)</span>, σ = ρ/ρ_SL <span class="chip dft">기본값</span>. 프로펠러는 축동력을 추력으로 바꾸므로 <b>고속에서 추력이 1/V로 빠진다</b> — 엔벨로프 상단을 정하는 것이 항력이 아니라 추력이 되고, 이 기체의 수평비행 범위가 해면 M0.20~0.60(연료 200 kg — 만재 400 kg면 M0.22~0.58)로 닫힌다(설계 천장 연료 200 kg ~5.5 km · 만재 ~3.8 km · 공허 ~7.5 km). 정지추력 6 kN은 시각화 모델 프로펠러 반경 0.45 m를 원판이론에 넣은 값(6.3 kN)의 라운드다</li>
