/** 서브시스템 하위 페이지 데이터 — 시뮬링크 "블록 더블클릭 → 내부 진입" 대응 (06 §1).
...
## 2계층 시각 문법 — 코드 대응은 색, 설명은 무채색 (06 §1)
  ...
  <text class="canvas-note" x="24" y="284">※ 입력은 SurfaceCommand 규약 (2,)를 그대로 받되 <tspan style="font-weight:700">평균만</tspan> 쓴다 · 교차속도 V_c = ηP/T_static = 66.7 m/s — 그 아래는 정지추력 상한, 위는 1/V</text>
        ...
        flow: {
          ...
          why: [
            ...
            "<b>1/V가 이 그림의 요점이다.</b> 상수 추력 모델은 고속에서 추력이 남아돈다고 말하지만 프로펠러는 정확히 거기서 힘이 빠진다. 그래서 엔벨로프 상단을 정하는 것이 항력이 아니라 <b>추력</b>이 되고, 예제 기체(200 kg급 — 정지추력 1.45 kN·축동력 49.3 kW — 아래 기본값의 무게비 상사에 추진만 1.45배)의 수평비행 범위가 해면 M0.09~0.28(연료 25 kg — 만재 50 kg면 M0.09~0.28)로 닫힌다(설계 천장 연료 25 kg ~7.3 km · 만재 ~6.1 km · 공허 ~8.6 km). 엔벨로프 탭의 스로틀 포화 전선이 대리 지표가 아니라 추력 한계인 이유가 이것이다 — 다만 그 선은 스로틀 <b>95%</b> 등고선이라(trim.py SAT_FRAC) 진짜 한계보다 설계 여유만큼 안쪽이다.",
...
<ul>
  ...
  <li>스로틀-추력: <span class="mono">T = δ·σ·min(T_static, ηP/V)</span>, σ = ρ/ρ_SL <span class="chip dft">기본값</span>. 프로펠러는 축동력을 추력으로 바꾸므로 <b>고속에서 추력이 1/V로 빠진다</b> — 엔벨로프 상단을 정하는 것이 항력이 아니라 추력이 되고, 예제 기체(200 kg급 — 정지추력 1.45 kN·축동력 49.3 kW — 아래 기본값의 무게비 상사에 추진만 1.45배)의 수평비행 범위가 해면 M0.09~0.28(연료 25 kg — 만재 50 kg면 M0.09~0.28)로 닫힌다(설계 천장 연료 25 kg ~7.3 km · 만재 ~6.1 km · 공허 ~8.6 km). 정지추력 6 kN은 시각화 모델 프로펠러 반경 0.45 m를 원판이론에 넣은 값(6.3 kN)의 라운드다</li>
...
<svg viewBox="0 0 1000 510" xmlns="http://www.w3.org/2000/svg">
  ...
  <text class="canvas-note" x="24" y="496">※ 합격기준(오버슈트·정착시간·경로오차)은 폐쇄망 Simulink 대조 시 확정 [04 §1] — 대조 절차는 엔진 밖(점선)</text>
</svg>`,
    flow: {
