 ...
 *
 * 사본을 **손으로** 만들지 않는 이유는 다른 lib과 같다: 그 모듈이 WebSocket 폴백과 422 오류 평탄화를
 * 들고 있고 `api.test.js`가 그것을 시험한다. 여기서 fetch를 다시 짜면 두 벌이 갈린다.
 * 다만 빌드는 그 모듈을 번들 **안에 복사**한다 — 모듈 상태(헤더 기체 선택, `lib/profile.js`)는 앱과
 * 공유되지 않는다. 그래서 이 번들은 기체를 받는 계산 라우트를 부르지 않는다(저장 결과·재생·자산만).
 * `web/js/lib/profile.test.js`가 그 사실을 원문에서 확인한다 (06 §8).
 *
