...
export const SURFACE_NOTES = {
  ...
  propellerDisplay:
    "프로펠러 회전은 집합 스로틀에 비례한 표시 값이며, 실제 회전수가 아닙니다 — "
    + "빠르면 블레이드 대신 반투명 원반으로 그립니다(모션블러 대용).",
  holdOnMissing:
 ...
 *
 * 시뮬에 회전수 모델이 없다. 있는 것은 정규화 스로틀 둘뿐이라, 그 평균(= 집합
 * 스로틀)을 회전속도에 선형으로 태운다. 실제 RPM이 아니고 캡션이 그렇게 말한다
 * (`SURFACE_NOTES`). 기체는 중심선 1기지만 신호가 둘인 것은 SurfaceCommand의
 * throttle 폭이 (2,)로 고정된 계약이기 때문이다 (engine plant/prop.py).
 *
 * 한쪽만 결측이면 남은 쪽을 쓰지 않는다 — 반쪽만 보고 집합 스로틀을 지어내면 그건
 * 없는 정보다. 둘 다 결측이면 null이고 호출측이 프로펠러를 멈춘 채 둔다.
 */
