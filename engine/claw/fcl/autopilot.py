...
데모 설계값 [기본값] (그 설계점은 프로펠러 추력 모델 이후 엔벨로프 밖이다 —
fcl/demo.py 머리말) — 고도 +100 m 오버슈트 8.3%, 속도 +10 m/s 3.7%, 헤딩 0.5 rad
...
class Autopilot(Block):
    ...
    PARAM_DEFS = (
        ...
        # **2.0을 유지한다 — 한 번 6.0으로 올렸다가 실측으로 되돌렸다.**
        #
        # 프로펠러 전환 뒤 +10 m/s 속도 스텝의 오버슈트가 18.7%로 커졌다(설계점
        # M0.4/1000 m). 처음엔 여유추력이 5,840 → 1,320 N으로 줄어 시간최적 가속이
        # 7.6 s가 됐으니 기준신호도 그만큼 느려야 한다고 보고 6.0으로 올렸다
        # (오버슈트 1.3%). **그 진단이 틀렸고, 고침도 손해였다.**
        #
        # 원인은 기준신호 기울기가 아니라 **적분 와인드업**이다:
        #   tau 2.0 · ki 0.03 → 오버슈트 18.7%, 스로틀 포화 8.5%
        #   tau 2.0 · ki 0    → 오버슈트 −2.1%, 스로틀 포화 4.8%  ← 포화는 남는데 사라진다
        # PID의 안티와인드업이 조건부 적분이 아니라 **적분기 상태를 출력범위로 클램프만**
        # 하는 형태라(blocks/controllers.py), 속도축은 out 범위가 [0,1]이어서 적분기가
        # "스로틀 100% 분량"까지 쌓인다. 게인을 낮추면 오버슈트가 되레 커지는 것
        # (0.15→0.04에서 18.7%→29.6%)도 같은 지문이다.
        #
        # 6.0은 그 증상을 **포화가 안 나게 해서 피한 것**이라 대가가 있었다:
        #   순항 승강타 활동량 de σ  tau 2.0 → 1.73° · tau 6.0 → 6.88° (피크 19.6°, 포화 20°)
        #   +10 m/s + 400 m 상승 오버슈트  tau 2.0 → 21.6% · tau 6.0 → 21.4% (거의 무효)
        # 순항 활동량은 "선회를 얹을 여유가 있는가"의 지표이고(test_landing), 상승을
        # 동반한 스텝은 6.0이 못 고친다. 즉 6.0은 한 케이스만 좋게 하고 나머지를 깎았다.
        #
        # 그 18.7%는 **안티와인드업 결함이 드러난 것**이었고, 그래서 PID를 조건부 적분으로
        # 고쳤다(blocks/controllers.py — 포화 방향으로 적분 정지). 지금 이 값(2.0)에서
        # 실측 오버슈트는 수평 7.32% · 상승 400 m 동반 7.59%다. tau를 다시 올릴 이유가
        # 없다: 원인이 사라졌고, 6.0이 깎던 순항 승강타 활동량도 그대로 지켰다.
