...
피치 루프를 리밋사이클에 넣었다). 전 비행영역(M0.25~0.8) 폐루프 고유치 안정 확인
완료. 1D mach 테이블이며 고도·연료 축 확장은 트림 격자 확보 후 [TBD].
...
# SingleEngine 중심선 1기). 중심선 1기는 좌우 추력차로 요 모멘트를 못 내므로 이
# 계수를 켜면 법칙은 요축을 돕는다고 믿는데 기체는 아무것도 안 하고, 스로틀이
# 상·하한에 붙은 구간에서는 좌우 클립이 비대칭이라 **평균이 밀려 러더가 추력을 깎는**
# 진짜 버그가 된다 (plant/prop.py SingleEngine 참조).
#
