"""제어법칙 구조의 중간 표현(IR) — 구조를 코드가 아니라 데이터로 적는다 (07 §1 · 02 §4).
...
class Graph:
    ...
    signal_types: {신호 이름: irtypes.Type} — **경계는 빠짐없이, 내부는 논거가 있는
    것만**. 다만 그 「빠짐없이」를 집행하는 것은 **`Graph`가 아니다** — 여기서 도는 것은
    형식 검사(모르는 이름·vector 거부)뿐이라 경계 선언이 빠진 그래프도 그대로 조립된다.
    필수로 만드는 것은 fcl 지역 규약(`fcl/graphs.py`의 `_types_for`·`_typed_outputs`)이고,
    저장소 전 그래프로 올리는 것은 별건이다 — 이유는 `_types_for` 독스트링에 있다.

    내부 노드는 기본이 미선언이되 **설계 논거가 붙는 사슬은 적는다** — 실물 fcl에서
    α 리미터 사슬과 스케줄이 낳는 게인이 그렇다(`fcl/graphs.py`의 `_limiter_types`·
    `_sched_gain_types`가 정본). 「내부는 전부 비어 있다」를 다음 단계의 전제로 삼으면
    안 된다 (07 §8).

    그래도 두 벌이 안 되는 것은 **방향** 때문이다. 규칙 추론이 들어오면 이 선언들은
    추론의 입력이 아니라 **대조 대상**이 된다 — 어긋나면 죽는 자리다. 손으로 베낀
    사본이면 갈라질 때 조용하지만, 추론이 맞는지 묻는 물음이면 갈라질 때 시끄럽다.

    비면 검사가 **아예 안 돈다**(기본값 None) — 그래서 그래프 하나씩 점진 도입이
    가능하다. 타입은 읽히기만 하고 아무것도 만들지 않으므로 생성 C는 이것으로 한
    바이트도 바뀌지 않는다 (07 §8 — 지문에도 안 들어간다).
    """
