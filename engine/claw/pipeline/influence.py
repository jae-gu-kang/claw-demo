...
def probe_value(
    ...
    값이 정확히 0인 자리(k_thr_turn·ki_hdg·k_diff_thr)만 상대 스텝이 성립하지 않으므로 그때만
...
# 선언 순서 = 그래프 지표 열의 표시 순서다 (배치가 declIdx로 폴백) — 평가 항목 재편의
...
METRICS = (
    ...
    MetricDef("min_pitch_authority_frac", "최소 피치 잔여권한", "-",
              ...
              tier="A", group="타면·권한"),
    MetricDef("thr_sat_frac", "추력 포화율", "-", ("thr_l", "thr_r"), "lower",
              "추력이 「최대치」에 붙어 있던 표본 비율 — 여기가 크면 추종 오차는 "
              "게인이 아니라 「에너지」가 원인이다(포화 뒤에서는 루프 게인이 "
              "아무 일도 안 한다). 하한(아이들)은 세지 않는다 — 강하에서 "
              "스로틀 0은 정상이라 같이 세면 종말 강하가 포화 100 %로 찍힌다",
              tier="A", group="타면·권한"),
    MetricDef("thr_margin_min", "최소 추력 여유", "-", ("thr_l", "thr_r"), "higher",
              "기동 중 남아 있던 가장 얇은 여유 1 − thr — 트림값만으로는 안 보인다"
              "(상승·선회가 여유를 먹는 것은 기동 중에 일어난다)",
              tier="A", group="타면·권한"),
    MetricDef("min_roll_authority_frac", "최소 롤 잔여권한", "-",
              ("alloc_roll_hi",), "higher",
              "엘레본 예산 배분의 롤 동적 한계 최솟값 / 예산 — 배분 미장착이면 없음",
              tier="A", group="타면·권한"),
    # 트림 몫과 기동 몫 (v1.07, 01 §4.1) — 트림 여유는 트림 해에서, 동적 편차는 런에서 잰다
    MetricDef("de_dyn_reserve_min_frac", "가용 동적 여유(엘레본)", "-", ("de", "da"), "higher",
              "min[(한계 − |δe_trim| − |δ(t) − δe_trim|) / 한계] — 트림은 되지만 기동 여유가 없는 점을 잡는다"
              "(δmax − max|δ|의 보수적 하한). 롤 사용 포함, 지상·레일 구간 제외",
              tier="A", group="타면·권한"),
    MetricDef("de_trim_frac", "트림 승강타 소모율", "-", ("de",), "lower",
              "|δe_trim| / 그 부호 쪽 엘레본 한계 — 트림이 이미 가져간 몫(트림 해의 수치, 런과 무관)",
              tier="B", group="타면·권한"),
    MetricDef("de_excursion_max", "엘레본 기동 편차 최대", "rad", ("de", "da"), "lower",
              "max|δ_엘레본(t) − δe_trim| — 기동이 가져간 몫. 마하가 크게 변하는 런은 δe_trim(M) 할당 표가 기준선",
              tier="B", group="타면·권한"),
    MetricDef("thr_trim_reserve", "트림 추력 여유", "-", ("thr_l", "thr_r"), "higher",
              "1 − thr_trim — 기동 몫은 이 값에서 최소 추력 여유(thr_margin_min)를 뺀 것",
              tier="B", group="타면·권한"),
    # ── 엔벨로프·보호 (판정 — 하드 게이트의 문턱은 criteria가 쥔다) ────────
    MetricDef("worst_stall_margin", "최악 실속마진", "rad", ("alpha_margin",), "higher",
              "임무 전체에서 α_stall−α의 최솟값 (02 §6.1)",
              tier="B", group="엔벨로프·보호"),
    MetricDef("alpha_trim_reserve", "트림 실속 여유", "rad", ("alpha",), "higher",
              "α_stall(M_trim) − α_trim — 기동 몫은 이 값에서 최악 실속마진을 뺀 것",
              tier="B", group="엔벨로프·보호"),
    ...
    # ── 임무·이착륙 (3단계 — 미션 프로파일 검증의 지표, 01 §3.3.1) ─────────
