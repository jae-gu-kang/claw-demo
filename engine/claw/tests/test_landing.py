...
def test_cruise_elevon_activity_stays_bounded(landed):
    ...
    이 수치가 곧 "선회를 얹을 여유가 있는가"다. **착륙 성공만 보는 회귀로는 이
    결함이 안 잡힌다**: 균일 상한 3.0은 M0.26 순항에서 승강타가 ±19.6°를 2.6 Hz로
    왕복하면서도(정착 de σ 7.8°·q σ 9.8°/s) 일곱 단계를 완주해 117.0 s에 접지한다.
    활동량 자체를 못박는 자리가 따로 필요한 이유다. (상한별 표는 fcl/demo.py의
    `_F_CAP` 주석이 정본이다. 균일 4.0은 **순항에 도달조차 못 하므로**(상승 중
    66 s 추락) 이 문턱의 대조군이 아니다 — 옛 기체의 "4.0에서 σ 10.6°"가 아니라
    3.0이 현행 기체의 파탄 직전 형상이다.)

    자리별 실측(4배 부스트): pitch.kp만 σ 11.0°, pitch.k_rate만 σ 12.1°,
    pitch.ki만 σ 3.3°(= 부스트 없음), 롤 3개 σ 3.3°. 피치 비례·레이트가 원인이다.
    **이 분해만은 프로펠러 전환 이전 기체의 것이다** — `_F_CAP` 주석에 같은 경고가
    있고, 결론(피치가 원인·롤은 무관)만 v0.74가 현행 기체에서 재확인했다.
