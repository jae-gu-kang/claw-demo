...
def test_guidance_mission_alt_capture(trim_design):
    ...
    # 오버슈트 한도 25 → 30 (v1.05). **성능이 나빠진 것을 핀으로 덮는 것이 아니라
    # 대가를 기록하는 것이다** — 갈라 낸 결과를 적어 둔다:
    #
    #   표 없음(스칼라 0.3만)            1219.77 m (오버슈트 19.77)
    #   값이 같은 상수표 → min = 0.3     1219.77 m (오버슈트 19.77)  ← 차이 0.0000
    #   실속 유도 표 (M0.593에서 0.266)  1227.53 m (오버슈트 27.53)
    #
    # 즉 포트화(구조)는 결백하고 원인은 **한계 값**이다. 이 설계점은 M0.593이라
    # 보호경계가 0.266으로 종전 상수 0.3보다 낮은데, **더 조였더니 오버슈트가 커졌다.**
    # 전형적인 적분 와인드업이다 — 상한이 낮으면 기체가 느리게 올라가 큰 오차 구간에
    # 오래 머물고, 적분기가 그 한계에 포화한 채 있다가 오차 부호가 바뀐 뒤에도 늦게
    # 풀린다. 고도 루프 게인이 theta_hi = 0.3 시절에 튜닝된 값이라는 신호다.
    #
    # 30을 고른 근거는 기동 기준 dh(pipeline/criteria.py, 표준 고도 스텝 30 m)이지
    # "그만하면 됐다"가 아니다. 27.53은 그 안이지만 여유가 2.5 m뿐이다.
    # [TBD] 고도 루프 재튜닝 또는 백캘큘레이션 안티와인드업 — 스케줄이 상한을 조이는
    # 구간에서 이 여유가 더 줄어든다.
    assert np.max(res.signals["h"]) < 1200.0 + 30.0
