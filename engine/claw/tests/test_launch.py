...
def test_elevons_sit_trailing_edge_up_on_the_rail(pitch_launched):
    ...
    타각 **크기**는 게인이 정한다 — 동압 스케줄 상한을 4.0에서 2.0으로 내리면서
    −0.31 언저리에서 −0.16 언저리로 얕아졌다(fcl/demo.py 주석의 리밋사이클 수정).
    이 테스트가 지키는 것은 **부호**이므로 문턱도 그에 맞춰 내린다. 0으로 두지는
    않는다 — "상향으로 자리잡았다"와 "간신히 음수"는 다른 사실이다.
    """
    _rail, res = pitch_launched
    s = res.signals
    on = s["on_rail"]
    assert (s["de"][on] < -0.10).all(), "레일 구간 내내 엘러본이 상향이어야 함"
