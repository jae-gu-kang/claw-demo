...
def test_pid_out_of_range_warm_start_is_clamped_on_the_first_step():
    ...
    **AP 기본값에 기대지 않는다** — 여기서 PID는 자기 ±0.3을 들고 온다. 종전 주석은
    "트림 θ 상한 0.35 > AP theta_hi 0.3이라 도달 가능"이라고 근거를 댔는데, v1.03에서
    theta_hi가 0.40으로 올라가 그 부등호가 뒤집혔다. 클램프가 무조건이어야 하는 이유는
    그 특정 조합이 아니라 **웜스타트가 어디서 오든 범위를 보장해야 한다**는 것이다
    (게인 스케줄이 구간마다 다른 한계를 주면 같은 일이 다시 생긴다).
