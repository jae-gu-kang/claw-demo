...
def test_eig_migration_is_symmetric(setup):
    ...
    lon_hi, _ = split_axes(lms.get(ac, trs[0.7]))
...
def test_distance_symmetric_when_classification_fails(setup):
    ...
    d_ab = model_distance(lms.get(ac, trs[0.3]), lms.get(ac, trs[0.7]), trs[0.3], trs[0.7])
    d_ba = model_distance(lms.get(ac, trs[0.7]), lms.get(ac, trs[0.3]), trs[0.7], trs[0.3])
