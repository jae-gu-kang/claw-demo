...
2. 끝나면 아래 **[재생 + 엔벨로프 감시]** 서랍이 저절로 열린다 — 재생 슬라이더와
   궤적 4면(입체·평면·측면·정면), 그 아래 시계열.
3. 그 옆 **[타면 사용]** 서랍 — 같은 시뮬에서 조종면을 얼마나 썼는지, 포화가
   있었는지.
...
| 시뮬레이션 | 미션·웨이포인트 편집 → 폐루프 시뮬 → 재생·궤적 4면 · **타면 사용**(타각 히스토그램·누적 초과·타각–타율 밀도 + 작동기 능력 상자) |
| Autocode | 현재 형상의 탑재 C 생성·열람 (통합/모듈별) |
| 검증 | 탑재 C를 DAL A 기준·VectorCAST 문법으로 검증 — 정적 규율 → 엄격 컴파일 → 유닛(파티션)·통합 비트 대조 → 라인/분기 + **MC/DC**(자체 계측, masking) → 유닛 그리드·커버리지 소스 뷰어·진리표·DO-178C 대응표·인쇄용 증적 보고서. 독립쌍이 수학적으로 부재한 조건은 분석 정당화로 명시, 툴 없는 환경은 사유와 함께 생략 |
