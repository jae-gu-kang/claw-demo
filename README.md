...
접근 보호는 배포 설정에 따라 둘 중 하나다:

- **로그인 화면**(블록도가 조립되는 그 화면)이 뜨면 — 공유받은 계정으로 로그인하거나
  [가입 신청]을 넣는다. 신청은 관리자가 승인해야 로그인된다.
- **브라우저 기본 팝업**이 뜨면(구 방식 Basic) — 아이디는 아무거나, 비밀번호는
  공유받은 것을 넣는다.
...
- 결과는 휘발성이다. 잠들거나 재배포되면 `server_data/`가 초기화되고, 깨어
  ...
  로컬로 돌릴 것.
- 기체 탭에서 만든 기체도 같은 이유로 휘발성이다 — 잠들거나 재배포되면 사라진다(예제
  기체는 엔진에 딸려 있어 남는다). 남길 기체는 [내보내기]로 JSON을 받아 두고 「가져오기」로
  되살린다. 서버가 이 사실을 `/api/health`의 `profile_store.volatile`로 알리고 기체 탭이 말한다.
...
> 공개 URL로 내놓아야 한다면 둘 중 하나를 설정한다(`server/claw_server/auth.py`):
>
> - `CLAW_ADMIN_PASSWORD=비밀번호` — **세션 모드**: 로그인 화면·계정·가입 승인·
>   관리자 화면(회원관리)이 켜진다. `CLAW_SESSION_SECRET`(없으면 재시작마다 전원
>   로그아웃)과, 계정을 재배포 후에도 남기려면 `CLAW_DB_URL`(Postgres — 없으면
>   `server_data/users.json`)을 함께 설정.
> - `CLAW_ACCESS_PASSWORD=비밀번호` — 공용 비밀번호 하나짜리 Basic Auth(구 방식).
>   둘 다 있으면 세션 모드가 이긴다.
...
1. **시뮬레이션** 탭에서 **[시뮬 실행]** 을 누른다. 입력을 채울 필요 없다 —
   ...
   (발사대 사출 → 상승 → 우선회 장주 → 접근 → 플레어 → 활주로 정지, 약 510초 미션)
...
웹 UI 탭이 설계 단계에 대응한다 — **왼쪽에서 오른쪽이 업무 순서**다
(구조와 영역 → 선형 설계 → 날려 보고 눈으로 확인 → 격자 판정 → 코드 → 코드 대조).
마지막 **결과**만 단계가 아닌 열람 자리다. 영향성이 Fail을 내면 처방이 설계변수를
다시 풀어 게인·자동 설계로 돌아간다 — 탭 줄은 직선이지만 작업은 되돌아간다.
...
| 시뮬레이션 | 미션·웨이포인트 편집 → 폐루프 시뮬 → 재생·궤적 4면 · **타면 사용**(타각 히스토그램·누적 초과·타각–타율 밀도 + 작동기 능력 상자). 나온 런 하나가 [가상환경에서 보기]·[영향성에서 진단]으로 그대로 인계된다 |
| 가상환경 | 그 런의 궤적을 실지형 3D 위에서 재생 — 지형·웨이포인트·카메라 4종. 목록에서 다른 런을 고르면 영향성이 이어받는 런도 그것이 된다 |
| 영향성 | 파라미터 하나가 제어법칙 IR과 설계 지표에 어디까지 번지는지 + **케이스 격자 전체로 6DOF 판정**(카드 7·판정 10) → 소견·처방·3단계 검증. 시뮬 런을 넘겨받아 자기 미션의 결함을 귀속하는 수동 진단도 여기 있다 |
| Autocode | 현재 형상의 탑재 C 생성·열람 (통합/모듈별) |
| 검증 | 탑재 C를 DAL A 기준·VectorCAST 문법으로 검증 — 정적 규율 → 엄격 컴파일 → 유닛(파티션)·통합 비트 대조 → 라인/분기 + **MC/DC**(자체 계측, masking) → 유닛 그리드·커버리지 소스 뷰어·진리표·DO-178C 대응표·인쇄용 증적 보고서. 커버리지는 파라미터 세트(요청 기체 이미지 + 값으로 꺼진 경로를 켠 이미지) 합산이고 비활성 경로는 목록으로 낸다. 독립쌍이 수학적으로 부재한 조건은 분석 정당화로 명시, 툴 없는 환경은 사유와 함께 생략 |
...
검증한다. **값은 C에 없다** — 게인·표·필터 계수는 파라미터 이미지(`flight/params/`)로 따로
나가고 생성 로더가 비행 전에 적재한다. 같은 템플릿이면 기체가 바뀌어도 C는 바이트 동일하고
이미지만 다르다(한 실행 파일로 세 기체 이미지를 돌려 대조한다). **Autocode 탭은 같은
생성기를 서버(`POST /api/codegen/flight`) 경유로 불러 브라우저에서 보여 준다** — 웹이 C를
조립하는 것이 아니라서 응답의 C가 커밋된 `flight/gen/`과 바이트 단위로 같다(서버 테스트가 대조).
